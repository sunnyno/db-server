...
class DBServiceTest {
    static final TestDataProvider QUERY_EXECUTOR = new TestDataProvider()
    ...
    void executeCreateDatabaseTest() {
        ...
        assertEquals("0", actualOutput.replace(ESCAPE_CHAR, "").trim())
    ...
    void executeInsertTest() {
        ...
        dbService.executeQuery(new BufferedInputStream(socket.getInputStream()), socket.getOutputStream())
        String actualOutput = new String(outputStream.toByteArray())
        assertEquals("1", actualOutput.replace(ESCAPE_CHAR, "").trim())
        QUERY_EXECUTOR.setUpOrTearDown("{\"type\" : \"drop_table\", \"database\" :\"test\", \"table\":\"user\"}",
                "{\"type\" : \"drop_database\", \"database\" :\"test\"}")
    }
